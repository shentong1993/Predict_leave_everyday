...
def generate_data_and_label(file_path, day_len):
    ...
    #label,lastLabel1,apply_Id,breakfast1,lunch1,dinner1,trainning1,speak1,greed1,pressure1,menstrual1,breakfast2,lunch2,dinner2,trainning2,speak2,greed2,pressure2,menstrual2
    feature_map = ['lastLabel1','apply_Id']
...
def generateClf(total_train_Datas_List , total_train_Labels_List):
    ...
    for i in range(clf_num):
        ...
        new_train_Datas_list = [person[2:] for person in total_train_Datas_List[i]]
...
#[{'termID': 261, 'term_num': 114, 'predict_day': 22 , 'camp_start_time': '2018-07-23 00:00:00', 'predict_data':[{'apply_Id':_ , 'term_num':_ ,}...]}, ,,]
def no_weight_predict_leave(no_weight_not_over_term_list):
    #no_weight_not_over_term_list
    #predict0_list =[{'term_num':_ , 'name':_}, ,,]
    predict0_list =[]
    ...
    for term_num_dic in no_weight_not_over_term_list:
        ...
        if day_len <=26 and day_len >=1:
            ...
            day = day_len
            file_path = './data/no_weight_dataset/no_weight_train_data_with_lastLabel/%dday_data.csv'%day
            trainDatas, trainLabels = generate_data_and_label(file_path,day_len=day)
            total_train_Datas_List, total_train_Labels_List = generate_balance_data(trainDatas, trainLabels,day,num_of_label1_list=1000)
...
if __name__ == '__main__':
    ...
    for day in day_list:
        ...
        file_path = './data/no_weight_dataset/no_weight_train_data_with_lastLabel/%dday_data.csv'%day
        trainDatas, trainLabels = generate_data_and_label(file_path,day_len=day)
        total_train_Datas_List, total_train_Labels_List = generate_balance_data(trainDatas, trainLabels,day,num_of_label1_list=3)
