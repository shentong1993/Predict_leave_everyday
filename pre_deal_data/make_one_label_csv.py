#第三个运行 生成labelData / label1 , label2 ,label3...
    ...
    for person in data_list:
        ...
        for Lastday in range(27):
            ...
            for i in range(Lastday + 1):
                ...
                temp_count += person['check%d' %(FromDate+i)]
                temp_count += person['speak%d' %(FromDate+i)]

                week = math.ceil((FromDate+ i)/ 7 )
                if person['week%dWeight' %week] != 0:
                    temp_count += 1

                total_list.append(temp_count)

            total_array = np.array(total_list)
            label = np.sum(total_array)


            if label == 0 :
                person['label%d' %(28 - Lastday -1)] = 0
            else:
...
# def main():
#
#     #一共打27个标签， 第一天判断第二天 ， 第一，二天判断第三天， 前27天判断第28天
#     #
#     #label27 的意思是 用前27天 预测之后跑没跑
#     getLabel()
#
